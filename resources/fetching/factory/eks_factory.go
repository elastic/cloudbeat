...
import (
	...
	k8sFetcher "github.com/elastic/cloudbeat/resources/fetching/fetchers/k8s"
...
var (
	eksRequiredProcesses = k8sFetcher.ProcessesConfigMap{"kubelet": {ConfigFileArguments: []string{"config"}}}
...
func NewCisEksFactory(log *logp.Logger, awsConfig aws.Config, ch chan fetching.ResourceInfo, le uniqueness.Manager, k8sClient k8s.Interface, identity *awslib.Identity) FetchersMap {
	...
	fsFetcher := k8sFetcher.NewFsFetcher(log, ch, eksFsPatterns)
	m[fetching.FileSystemType] = RegisteredFetcher{Fetcher: fsFetcher}

	procFetcher := k8sFetcher.NewProcessFetcher(log, ch, eksRequiredProcesses)
	m[fetching.ProcessType] = RegisteredFetcher{Fetcher: procFetcher}

	kubeFetcher := k8sFetcher.NewKubeFetcher(log, ch, k8sClient)
