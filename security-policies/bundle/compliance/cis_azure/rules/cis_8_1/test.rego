...
test_violation if {
	eval_fail with input as test_data.generate_key_vault_rbac({"vaultKeys": [test_data.generate_key_vault_extension_key({"enabled": true})]})
	eval_fail with input as test_data.generate_key_vault_rbac({"vaultKeys": [
...
test_pass if {
	eval_pass with input as test_data.generate_key_vault_rbac({"vaultKeys": [
		test_data.generate_key_vault_extension_key({"enabled": true, "exp": 1768523299}),
