package compliance.cis_azure.rules.cis_8_3
...
test_violation if {
	eval_fail with input as test_data.generate_key_vault_rbac({"vaultSecrets": [test_data.generate_key_vault_extension_key({"enabled": true})]})
	eval_fail with input as test_data.generate_key_vault_rbac({"vaultSecrets": [
...
test_pass if {
	eval_pass with input as test_data.generate_key_vault_rbac({"vaultSecrets": [
		test_data.generate_key_vault_extension_key({"enabled": true, "exp": 1768523299}),
