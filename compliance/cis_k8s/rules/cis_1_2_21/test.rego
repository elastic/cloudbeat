...
test_violation {
	test.assert_fail(finding) with input as rule_input("api_server", "--profiling=true")
	test.assert_fail(finding) with input as rule_input("api_server", "")
}

test_pass {
	test.assert_pass(finding) with input as rule_input("api_server", "--profiling=false")
}
