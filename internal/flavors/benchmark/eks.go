...
func (k *EKS) getEksAwsConfig(ctx context.Context, cfg *config.Config, logger *clog.Logger) (awssdk.Config, *cloud.Identity, error) {
	if cfg.CloudConfig == (config.CloudConfig{}) || cfg.CloudConfig.Aws.Cred == (aws.ConfigAWS{}) {
		// Optional for EKS
		return awssdk.Config{}, nil, nil
	}

	awsCfg, err := awslib.InitializeAWSConfig(cfg.CloudConfig.Aws.Cred, logger)
	if err != nil {
