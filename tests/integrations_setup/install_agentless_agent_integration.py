...
1. Create an agent agent policy.
...
if __name__ == "__main__":
    ...
    logger.info(f"Creating agentless-agent {INTEGRATION_NAME} linux manifest")
    # Write the rendered content to a file
    with open(Path(__file__).parent / "cspm-linux.sh", "w", encoding="utf-8") as cspm_file:
        cspm_file.write(rendered_content)

    logger.info(f"Installation of agentless-agent {INTEGRATION_NAME} integration is done")
