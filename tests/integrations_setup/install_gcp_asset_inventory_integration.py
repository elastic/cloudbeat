#!/usr/bin/env python
"""
This script installs Cloud Asset Discovery (Asset Inventory) GCP integration

The following steps are performed:
1. Create an agent policy.
2. Create a Cloud Asset Discovery (Asset Inventory) GCP integration.
...
CAD_GCP_EXPECTED_AGENTS = 1
DEPLOYMENT_MANAGER_CONFIG = "../../deploy/deployment-manager/config.json"

cad_gcp_deployment_manager_config = Path(__file__).parent / DEPLOYMENT_MANAGER_CONFIG
INTEGRATION_NAME = "Cloud Asset Discovery GCP"
PKG_DEFAULT_VERSION = VERSION_MAP.get("asset_inventory_gcp", "")
INTEGRATION_INPUT = {
    "name": generate_random_name("pkg-asset-inventor-gcp"),
    "input_name": "asset_inventory_gcp",
    "deployment": "gcp",
}
AGENT_INPUT = {
    "name": generate_random_name("cad-gcp"),
}
...
if __name__ == "__main__":
    ...
    if not package_version:
        logger.error("Could not resolve cloud_asset_inventory package version from Fleet (Kibana may be unavailable).")
        raise SystemExit(1)
    ...
    if version.parse(package_version) >= version.parse("1.0"):
    ...
    with open(cad_gcp_deployment_manager_config, "w") as file:
