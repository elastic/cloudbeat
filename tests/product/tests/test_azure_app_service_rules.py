"""
CIS Azure App Service rules verification.
...
from product.tests.data.azure import (
    azure_app_service_test_cases as azure_app_service_tc,
)
...
@pytest.mark.cspm_azure_app_service_rules
def test_azure_app_service_rules(
    cspm_client,
    cloudbeat_agent,
    rule_tag,
    case_identifier,
    ...
    evaluation = get_ES_evaluation(
        ...
        resource_identifier=app_service_identifier,
    )
...
register_params(
    test_azure_app_service_rules,
    Parameters(
        ("rule_tag", "case_identifier", "expected"),
        [*azure_app_service_tc.test_cases.values()],
        ids=[*azure_app_service_tc.test_cases.keys()],
    ),
)
