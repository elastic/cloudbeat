...
run-tests target='default':
  helm test {{TESTS_RELEASE}} -n {{NAMESPACE}} --kube-context kind-kind-multi --timeout {{TESTS_TIMEOUT}} --logs 2>&1 | tee {{TEST_LOGS_DIRECTORY}}/{{target}}-$(date +"%d-%m-%y-%H-%M-%S").log
