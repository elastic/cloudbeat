...
##  or staging). Reads the version from version/version.go, resolves the
##  staging qualifier via qualifier.sh, and emits a step that runs
##  elastic/dra-prep + (on non-dry-run builds) a summary annotator and the
##  unified-release-dra-processing trigger.
##
##  This script is invoked once per workflow from pipeline.yml and leverages
##  elastic/dra-prep-buildkite-plugin to upload DRA artifacts.
##
...
# plugin runs but does not upload to GCS, and we skip both the annotator
