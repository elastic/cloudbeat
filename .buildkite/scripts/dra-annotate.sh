...
##  the workflow's summary at the dra-prep plugin's temp upload path.
##
...
buildkite-agent artifact download "artifacts/dra/cloudbeat/*/manifest-*.json" . --step "dra-prep-${WORKFLOW}"
manifest=$(find artifacts/dra/cloudbeat -name "manifest-*.json" | head -1)
