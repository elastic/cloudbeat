...
if __name__ == "__main__":
    ...
    package_policy_id = create_kspm_unmanaged_integration(cfg=cnfg.elk_config,
                                                          pkg_policy=package_data,
                                                          agent_policy_id=agent_policy_id)

    save_state(cnfg.state_data_file, [{"pkg_policy_id":package_policy_id,
                                       "agnt_policy_id": agent_policy_id}])
    manifest_params = Munch()
    manifest_params.enrollment_token = get_enrollment_token(cfg=cnfg.elk_config,
